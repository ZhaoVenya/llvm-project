...
// OneTargetLowering::OneTargetLowering() 这个是OneTargetLowering class的构造函数
// 在创建这个类的时候，构造函数会自动被调用
OneTargetLowering::OneTargetLowering(const TargetMachine &TM, const OneSubtarget &STI)
...
// 这个函数负责处理函数**调用（Call）**指令的降低。它是被调用方视角。
SDValue OneTargetLowering::LowerCall(CallLoweringInfo &CLI, SmallVectorImpl<SDValue> &InVals) const {
...
SDValue OneTargetLowering::LowerFormalArguments( SDValue Chain, CallingConv::ID CallConv, 
...
// 这个函数负责处理函数返回（Return）的降低，即被调用函数如何将返回值传递给调用者。
SDValue OneTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool IsVarArg,
...
// 这是 TargetLowering 类中最核心、最通用的函数之一，负责处理自定义操作的降低。
SDValue OneTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
    switch (Op.getOpcode()) {
        ...
        default: llvm::llvm_unreachable_internal("unknown op");
    }
    return SDValue();
}
...
// 这是一个辅助函数，通常由 LowerOperation 调用，
// 用于处理 LLVM IR 中的**全局地址（Global Address）**的降低。
/// GlobalAddress -> HI / LO
...
// 这是一个辅助函数，通常由 LowerOperation 调用，用于处理常量的降低
...
// 这个函数提供了一种机制，用于为你的后端自定义的 SDNode 操作码返回其对应的字符串名称。
const char *OneTargetLowering::getTargetNodeName(unsigned Opcode) const {
    switch (Opcode) {
        case OneISD::RET_GLUE: return "OneISD::RET_GLUE";

        case OneISD::Call: return "OneISD::Call";

        case OneISD::HI: return "OneISD::HI";

        case OneISD::LO: return "OneISD::LO";
